...
function PageMain() {
	...
	if(loginCheck($_COOKIE['username'], $_COOKIE['password'])) {
		...
		$skin = new skin('disabledaccount/rows'); $rows = '';
		...
		$skin = new skin('disabledaccount/profile'); $profile = '';
		...
		$skin = new skin('disabledaccount/top'); $top = '';
	...
	$TMPL['url'] = $confUrl;
	$TMPL['title'] = 'Disabled Account - '.$resultSettings[0];

	$skin = new skin('disabledaccount/content');
	...
	if(isset($_POST['reactivate'])) {
				$password = md5($result[2]);
					...
					if($_POST['username'] == $_COOKIE['username'] && md5($_POST['password']) == $_COOKIE['password']) {
						...
						$query = sprintf("UPDATE `users` SET disabled = 0 WHERE username = '%s'", 
										mysql_real_escape_string($_COOKIE['username']));
						mysql_query($query);
						
						header("Location: /index.php?a=stream&m=re");
