...
function PageMain() {
	...
	$skin = new skin('explore/latest'); $latest = '';
		
	$queryLatest = "SELECT * FROM users WHERE image <> '' ORDER BY rand() DESC LIMIT 3";
	$resultLatest = mysql_query($queryLatest);
	
	while($TMPL = mysql_fetch_assoc($resultLatest)) {
		
		$TMPL['url'] = $confUrl;
		$TMPL['image'] = (!empty($TMPL['image'])) ? '<img src="'.$confUrl.'/uploads/avatars/'.$TMPL['image'].'" width="48" height="48" />' : '<img src="http://www.gravatar.com/avatar/'.md5($TMPL['email']).'?s=48&d=mm" />';md5($result[3]);
	...
	$TMPL['latest'] = $latest;
		...
		$skin = new skin('explore/rows'); $rows = '';
		...
		while($TMPL = mysql_fetch_assoc($resultMsg)) {
			...
			if (!empty($TMPL['suspended'])) {
				$TMPL['image'] = '<img src="'.$confUrl.'/images/suspended.png" width="50" height="50" />';	
			} else {
				$TMPL['image'] = (!empty($TMPL['image'])) ? '<a href="'.$confUrl.'/profile/'.$TMPL['username'].'"><img src="'.$confUrl.'/uploads/avatars/'.$TMPL['image'].'" width="50" height="50" /></a>' : '<a href="'.$confUrl.'/profile/'.$TMPL['username'].'"><img src="http://www.gravatar.com/avatar/'.md5($TMPL['email']).'?s=50&d=mm" /></a>';md5($result[3]);
			}
			...
			if ($TMPL['disabled'] == 1) {
				...
				<img src="http://gravatar.com/avatar/00000000000000000000000000000000?d=mm&f=y" width="50" height="50" /></a>';	
			} 
		...
		$skin = new skin('explore/profile'); $profile = '';
		...
		$TMPL['name'] = (!empty($result[4])) ? ''.$result[4].'' : '';
		$TMPL['url'] = $confUrl;
		...
		$TMPL['bio'] = (!empty($result[7])) ? ''.$result[7].'' : 'No given Infomation';
		$TMPL['verify'] = $result[20];
	...
	$TMPL['url'] = $confUrl;
	$TMPL['title'] = 'Explore - '.$resultSettings[0];
