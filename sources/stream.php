...
function PageMain() {
	...
	if(loginCheck($_COOKIE['username'], $_COOKIE['password'])) {
		...
		$skin = new skin('stream/rows'); $rows = '';
		...
		/* TEXT TO EMOTICON
			$messageEmoticons = $resultMsg; 
			$chars = array(":D", ":zethcrew:");
			$icons = array('<img src="https://cdn1.iconfinder.com/data/icons/16x16-free-toolbar-icons/16/5.png">', '<img src="https://cdn1.iconfinder.com/data/icons/circular%20icons/smiley_flat.png">');
			$TMPL['message'] = str_replace($chars, $icons, $TMPL['message']);
		*/
		...
		while($TMPL = mysql_fetch_assoc($resultMsg)) {
			...
			$TMPL['delReply'] = ($TMPL['username'] == $_COOKIE['username']) ? '<div class="delete-button"><img src="'.$confUrl.'/images/icons/delete_message.png" /><a href="'.$confUrl.'/index.php?a=stream&d='.$TMPL['id'].'">Delete</a></div>' : '<div class="reply-button"><img src="'.$confUrl.'/images/icons/reply.png" />Reply</div>';
				...
				if($TMPL['repliedTo'] != 0) {
					$TMPL['conversation'] = '<div class="conversation-button"><a href="'.$confUrl.'/index.php?a=message&m='.$TMPL['repliedTo'].'">View conversation</a></div>';
				} else if($TMPL['repliedTo']){
					$TMPL['conversation'] = ''; 
				}
		...
		$skin = new skin('stream/profile'); $profile = '';
	...
	$skin = new skin('stream/userlist'); $followerList = '';
	...
	$TMPL['url'] = $confUrl;
	$TMPL['title'] = 'Stream - '.$resultSettings[0]; 
	
	

	$skin = new skin('stream/content');
