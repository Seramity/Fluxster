...
function PageMain() {
	...
	if(loginCheck($_COOKIE['username'], $_COOKIE['password'])) {
		...
		$skin = new skin('bundle/rows'); $rows = '';
		...
		$skin = new skin('bundle/profile'); $profile = '';
		// Get Profile Data
	...
	if($_GET['b'] == 'manage') {
				$skin = new skin('bundle/manage'); $bundlePage = '';
				
				
				$bundlePage .= $skin->make();
	
	} elseif($_GET['b'] == 'my') {
				$skin = new skin('bundle/my'); $bundlePage = '';
				
				
				$bundlePage .= $skin->make();
	} elseif($_GET['b'] == 'create') {
				$skin = new skin('bundle/create'); $bundlePage = '';
				
							
				if(isset($_POST['create'])) {
					
					if(isset($_POST['bundleName']) && isset($_POST['catagory']) && isset($_POST['name']) && isset($_POST['privacy'])){
						$createQuery = sprintf("INSERT into `bundles` (`user`, `name`, `date`, `description`, `private`, `catagory`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s');",
								mysql_real_escape_string($_COOKIE['username']),						
								
								mysql_real_escape_string($_POST['bundleName']),	
								
								date("Y-m-d H:i:s"),
								
								mysql_real_escape_string(strip_tags($_POST['description'])),
								
								mysql_real_escape_string($_POST['privacy']), 
								
								mysql_real_escape_string($_POST['catagory']));
				...
				$skin = new skin('bundle/main'); $bundlePage = '';
	...
	$TMPL['url'] = $confUrl;
	$TMPL['title'] = 'Bundle - '.$resultSettings[0];

	$skin = new skin('bundle/content');
