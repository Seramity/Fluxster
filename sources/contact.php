...
function PageMain() {
	...
	$resultSettings = mysql_fetch_row(mysql_query(getSettings($querySettings)));
		...
		$skin = new skin('contact/rows'); $rows = '';
		...
		$skin = new skin('contact/profile'); $profile = '';
	...
	$TMPL['url'] = $confUrl;
	$TMPL['title'] = 'Contact - '.$resultSettings[0];

	$skin = new skin('contact/content');
	...
	if(loginCheck($_COOKIE['username'], $_COOKIE['password'])) {
		$TMPL['loggedInUsername'] = 'value="'.$result[1].'" disabled="true"'; 
		$TMPL['loggedInEmail'] = 'value="'.$result[3].'" disabled="true"';
	} else {
		$TMPL['loggedInUsername'] = ''; 
		$TMPL['loggedInEmail'] = ''; 
	}
